...
runDETLEARNmodel = function(corpus, analysis, morphology, model, withImputation, windowSize, skip, n.adapt=2000, n.update=2000, n.iter=5000,n.chains=5, thinning=5, countTrace=T,datasetName, inputDirName, outputDirName, parallelize){
	...
	if (model == 'linking'){		
		...
		jobInputs = lapply(1:length(Nr.combined.observed.list),function(x){return(list(input = Nr.combined.observed.list[[x]], wordFreqs = adultNFreqs, modelname = 'JAGS_models/linking-model-with-imputation6.bug', 	n.iter=n.iter, countTrace = countTrace, thinning=thinning, n.adapt=n.adapt, n.update = n.update, n.chains = n.chains, model='linking'))})	
		...
		jobInputs = lapply(1:length(Nr.combined.observed.list),function(x){return(list(input = Nr.combined.observed.list[[x]], wordFreqs = adultNFreqs, modelname = 'JAGS_models/lmwi_splitHalf5.bug', n.iter=n.iter, countTrace = countTrace, thinning=thinning, n.adapt=n.adapt, n.update = n.update, n.chains = n.chains, model='linkingChange'))})
	}
	...
	t1 = try({ 			
		if(jobInput[['model']] == 'linkingChange'){
			df = jobInput$input
	  		...
	  		#pull out the indices that we need for the iterators in the JAGS model                    		  		
